...
After examining the ACF and PACF plots [Appendix ? Figure ?], we realized that the patterns we observe in the Figure ? might be evidence of AR or MA structures. In addition, the decomposition plot of the residuals [Appendix ? Figure ?] show seasonal patterns, suggesting that we might need to incorporate seasonal differencing. To account for these structures, we choose to use a regression with Seasonal Autoregressive Integrated Moving Average (SARIMA) errors [@ARIMA_errors_traffic_study; @Regression_with_ARIMA_errors_penn; @Regression_with_ARIMA_errors_Hyndman].
...
$$n_i = \frac{1}{(1-\phi_1 L)(1-L^{12})} \ a_i$$ where $\phi$ is the autoregression parameter, $a_i$ is assumed to be white noise [@ARIMA_errors_traffic_study], and $L$ is the lag operator (i.e. $L^k\ y_t = y_{t-k}$) [@Lag_operator].
...
kable(vif.m0.table, caption = "First OLD Model VIF Outputs", col.names = cbind("Variable", "VIF Value"), align = c('l', 'r'))
```
...
plot(decompose(q1.lm1.residuals)$seasonal,
     ...
     xaxt="n",
     main = "Residuals Show Seasonal Patterns",
     cex.main=0.85,
     cex.axis=0.8,
     cex.lab=0.8)

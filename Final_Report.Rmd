...
The dataset used in this analysis is the publicly available Kaggle Avocado Prices dataset, which credited the Hass Avocado Board for the collection and release of the data. This dataset contains 13 variables encompassing the per-unit prices, total sales volumes, regions, and sizes of Hass avocados, a cultivar of avocados. Specifically, the per-unit prices are the average retail price of individual avocados. The total sales volume is the count number of individual avocados sold. Considering the magnitude of this variable is unusually large (an 8-digit value), we recoded it in millions of avocados sold to keep it as a value under 100.

Data from the start of 2015 to the end of the first quarter of 2018 are available. Each entry represents one observation from one region in the U.S. during one week. To smooth out the data, we aggregated the weekly raw data into monthly values by taking the mean. The following analyses are based on the monthly version of the data. Additional analyses on the weekly version of the data yield similar results and are included in !!!!!!!!!!!!!!!!!!!Appendix ??.

To enrich this dataset, we join it with selected macroeconomic data. Specifically, we included the unadjusted monthly unemployment rate released by the U.S. Bureau of Labor Statistics and the unadjusted monthly Consumer Price Index (CPI) for all items in the U.S. (assuming the index 2015 is 100) provided by the Federal Reserve Bank. The rationales for selecting these external data will be explained in the methodology section.

## Research Goals

For this project, we focus on Hass conventional (non-organic) avocados sold in the U.S. We plan to conduct the forecast from April 2017 to March 2018, which corresponds to the last year in our dataset. We make predictions on one full year because we suspect that there might be some seasonal patterns in avocado sales volume and prices that repeat annually. We choose to use the last year in our dataset because we would like to compare our predicted values to the actual results.
...
For the first research question, our numerical response variable is the total count number (in millions) of conventional avocados sold in the U. S. The main predictor of interest is the per-unit retail price of avocados in dollars. Intuitively, we might expect higher sales volume when the price is low and vice versa. The data supports this intuition. As shown in Figure 1, we largely see higher sales volume associated with lower per-unit prices. The gray line represents the OLS model $\text{Total Volume}_i = \beta_0 + \beta_1 \  \text{Per-Unit Price}_i$, and the gray region shows its 95% confidence interval. We observe a potential inverse relationship between the total sales volume and the pre-unit price of avocados.
...
Another independent variable of interest is the unemployment rate. In economics, the unemployment rate is defined as the proportion of unemployed individuals in the labor force [@Unemployment_definition]. Since unemployed persons usually lose purchasing power, a high unemployment rate can potentially be associated with a decline in the retail sector [@Unemployment_implication]. In addition to the unemployment rates, the Consumer Price Index (CPI) is also a commonly used macroeconomic indicator. Specifically, the CPI is a measure of ``the average change over time in the prices paid by urban consumers for a market basket of consumer goods and services" ''[@BLS_CPI]. In other words, CPI can be used to assess price changes associated with the cost of living [@CPI_implication]. We hope to potentially control for some measure of the changes in the price level of consumer goods overall [@CPI_price_level].
...
$$\text{Total Volume}_i = \beta_0 + \beta_1 \ \text{Per-Unit Price}_i + \beta_2 \ \text{Unemployment Rate}_i + \beta_3 \ \text{CPI}_i  + \epsilon_i, \: \epsilon_i \sim \mathcal{N}(0, \sigma^2)$$ 

However, an examination of model assumptions reveal that a few of the assumptions of an OLS model are violated. First, when checked for evidence of multicollinearity, the VIF values are all less than 5, which seem to fall into the commonly acceptable range [@VIF_range]. However, as shown in the correlation plot [Appendix?, Figure?], the correlation between unemployment rate and CPI turns out to be -0.824, which is relatively high. As a reference, the frequently used threshold for the indication of potential multicollinearity is a correlation of magnitude 0.7 [@Common_cutoff]. This high magnitude of correlation might not be a surprise to economists. The CPI can be used as a measure of inflation, and inflation and unemployment are traditionally said to be inversely correlated [@CPI_unemployment_relationship]. To address the issue of multicollinearity, we have tried removing one of the highly correlated predictors. Removing one predictor indeed changes the OLS model outputs. Specifically, after removing CPI, the estimated coefficient of the unemployment rate changes from a positive value to a negative value. This updated result seems to conform to our expectation that higher unemployment rate is associated with lower sales volume. In addition, the magnitude of correlation between CPI and per-unit price (0.544) is slightly higher than the magnitude of correlation between unemployment rate and per-unit price (0.523). Thus, we decided to remove CPI as a predictor and attempt the following model:

$$\text{Total Volume}_i = \beta_0 + \beta_1 \ \text{Per-Unit Price}_i + \beta_2 \ \text{Unemployment Rate}_i  + \epsilon_i, \: \epsilon_i \sim \mathcal{N}(0, \sigma^2)$$ 

However, even in this updated model, some assumptions of this OLS model are still violated. Most notably, the residuals seem to have a non-constant variance [Figure 2(a)] and also seem to be correlated over time, or autocorrelated [Figure 2(b)].
...
Actually, the patterns we observe in Figure ? is somewhat expected since we are analyzing time series data. A time series is a sequence of values taken at successive equally spaced points in time [@Time_series_definition]. In this case, the time period is a month. When we use regression to model time series data, the error terms are often autocorrelated [@ARIMA_errors_traffic_study]. In addition to observing the residual plots, we typically use the Autocorrelation Function (ACF) plots and the Partial Autocorrelation Function (PACF) plots to detect autocorrelation in the residuals [@ACF_PACF]. We can take autocorrelation into account by adding structures such as autoregression (AR), moving average (MA), and/or using techniques such as differencing.

After examining the ACF and PACF plots [Appendix ? Figure ?], we realized that the patterns we observe in the Figure ? might be evidence of AR or MA structures. In addition, the decomposition plot of the residuals [Appendix ? Figure ?] show potential seasonal patterns, suggesting that we might need to incorporate seasonal differencing. To account for these structures, we choose to use a regression with Seasonal Autoregressive Integrated Moving Average (SARIMA) errors [@ARIMA_errors_traffic_study; @Regression_with_ARIMA_errors_penn; @Regression_with_ARIMA_errors_Hyndman].

### Regression with SARIMA Errors

A regression with SARIMA errors is similar to an OLS model except for a different error structure [@SARIMA_with_errors]. Specifically, we can define our new model as:

$$\text{Total Volume}_i = \beta_0 + \beta_1 \ \text{Per-Unit Price}_i + \beta_2 \ \text{Unemployment Rate}_i + n_i$$

The $n_i$, or the errors of the regression, is modeled by SARIMA. SARIMA models are usually concisely represented as $(p, d, q) \times (P, D, Q)_s$, where the lowercase letters correspond to the non-seasonal components and the uppercase letters correspond to the seasonal components. Specifically, the parameter $p$ is the number of time lags for the autoregressive (AR) model, $d$ is the degrees of differencing, and $q$ is the orders of the moving-average (MA) model. The uppercase $P, D, Q$ denote the autoregressive, differencing, and moving average terms for the seasonal part of the ARIMA model. Lastly, $s$ denotes the time period, in this case, $s$ is 12, denoting the 12 months in a year in our monthly version of the data.[@ARIMA_Notation]. The values of the parameters are determined using principles from the Box–Jenkins method [@ARIMA_Box_Jenkins]. We choose to describe the error terms by a $(1, 0, 0) \times (0, 1, 0)_{12}$ process; the parameter selection procedure is described in Appendix ?.

Thus, we can express the error terms as follows:

$$n_i = \frac{1}{(1-\phi_1 L)(1-L^{12})} \ a_i$$ where $\phi_1$ is the autoregression parameter, $a_i$ is assumed to be white noise [@ARIMA_errors_traffic_study], and $L$ is the lag operator (i.e. $L^k\ y_t = y_{t-k}$) [@Lag_operator].

The main assumption to check for a SARIMA model is that the residuals indeed follows a white noise process. Specifically, this means that the residuals should largely have a mean of zero, have constant variance and also be uncorrelated [@SARIMA_assumptions]. As shown in Figure?, these assumptions all seem to be satisfied.

We used the \texttt{Arima()} function in the R \texttt{forecast} to fit this regression model with SARIMA errors and the \texttt{predict()} function to make the forecast. Further details are included in Appendix ?. The data from January 2015 to February 2017 are used to fit the model, while the forecast is made from April 2017 to March 2018.


## Results
```{r fig.height = 4, fig.width = 5.5, fig.cap="Predicting the Monthly Avocado Sales Volume from 2017-2018"}
...
Figure ? shows the prediction results. The red dashed line shows the predicted sales volume from April 2017 to March 2018 while the black line shows the actual sales volume during the same time period. The green dotted line is the sales volume in the previous year (i.e. from April 2016 to March 2017). If we do not fit a model and simply use the sales volumes from the previous year, we could still roughly approximate the sales volumes. However, our predicted values seem to match more closely with the actual sales volumes. A commonly used metric to evaluate forecast accuracy is the Root Mean Square Error (RMSE) [@Forecast_evaluation]. RMSE is the standard deviation of the prediction errors [@RMSE_definition]. In general, we prefer smaller RMSE values. 

The RMSE of our prediction is approximately 2.085, which is smaller than an RMSE of 4.115 if we simply use the previous year's sales values. In the context of this analysis, this means that 2.085 is the square root of the mean of squared differences between the actual sales volumes and our predicted sales volumes [@RMSE_interpretation]. One advantage of the RMSE metric is that it has the same units as the response variable [@RMSE_units]; in this case, the unit is millions of avocados. While an RMSE of 2.085 million avocados is a considerable quantity, we recall that our response variable is the total number of avocados sold in the U.S. as a whole. In addition, as shown in Figure ?, the sales volume in one year can range from around 25 to 45 million avocados. Thus, we consider an RMSE of 2.085 million avocados an acceptable value in this context and conclude that we have made a reasonable prediction using our limited data.

It is worth noting that our predicted values under-predict the sales volume during the entire prediction time period. This might be due to other factors such as the changes in the supply volume and/or the quality of the supply.

The parameter estimates of the SARIMA model are usually not directly interpretable. We have included the values in Appendix ?, Table ? for reference.

\newpage
# Research Goal \texttt{\#}2: Predicting Per-Unit Price

## Variables and EDA
Our numerical response variable for the second research goal is the average per-unit retail price of conventional avocados in the U.S. It is very tempting to include the total sales volume as the predictor given the potential inverse relationship shown in Figure 1. However, we recognize that we would not know the sales volume when we do not know the retail prices. Thus, it does not make sense to use the sales volume to predict the prices. One counter-argument is that we can potentially use the sales volume as a proxy for demand. While this is not entirely convincing, we created a model which includes the sales volume as a predictor in a sensitivity analysis in Appendix ?. For the main analysis, we choose to proceed without the sales volume variable.
...
Similar to our arguments in Section 2, we would like to account for these structures using a regression with Seasonal Autoregressive Integrated Moving Average (SARIMA) errors. 

###Regression with SARIMA Errors

From Section 2, we know we can express our regression with SARIMA errors as follows:

$$\text{Per-Unit Price}_i = \beta_0 + \beta_1 \ \ \text{Unemployment Rate}_i + n_i$$ 

where the information remaining in the error terms are modeled by SARIMA $(p, d, q) \times (P, D, Q)_s$. We use the Box-Jenkins method [@ARIMA_Box_Jenkins] detailed in Appendix ? to select the values of $p, d, q$ and $P, D, Q$. We thus have:

$$n_i = \frac{1}{(1-\phi_1 L)(1-L^{12})} \ a_i$$ where $\phi$ is the autoregression parameter, $a_i$ is assumed to be white noise, and $L$ is the lag operator (i.e. $L^k\ y_t = y_{t-k}$) [@Lag_operator].

Figure ? in Appendix ? shows that our the residuals of our regression with SARIMA errors model follows a white noise process, satisfying the assumptions of zero mean, constant variance, and uncorrelated residuals [@SARIMA_assumptions].
...
## Results
```{r fig.height = 4, fig.width = 5.5, fig.cap="Predicting the Per-Unit Price of Avocados from 2017-2018"}
...
The predicted results are shown in Figure?. The black line shows the actual per-unit retail price of avocados from April 2017 to March 2018, the dashed red line represents the predicted results, and the green dotted line is the per-unit price from the previous year (April 2016 to March 2017). 

First, we notice that the predicted values do not match the actual results very closely. The seasonal trend is largely captured, but there seems to be a slight "delay" in the predicted values. For instance, according to actual values, the per-unit price peaked in September 2017, while the highest predicted value occurs in October 2017. The price drop between September and October 2017 in the actual values is somewhat mirrored in our prediction, although the drop is shifted one month back. 

Actually, our predicted values are closer to last year's prices than to the actual prices. This is likely due to our lack of external data. Perhaps we have not accounted for some important changes in the production of Hass avocados in our model. For example, it is said that the Hass cultivar has a tendency to bear well in alternate years. After a season with a relatively low yield, due to factors such as cold weather, the Hass avocado trees tend to produce more abundantly the next season. When the crop depletes the stored nutrients, the yield in the upcoming season would be reduced, thus establishing an alternate bearing pattern [@Health_benefits]. In future analyses, if we would have access to actual production data and/or a longer time period (i.e. more than three years of data), we would potentially be able to make a better forecast. 

Nonetheless, given our current data and model, our predicted values are closer to the truth than that of simply using the prices from the previous year. Specifically, the RMSE of our prediction is 0.167, which is smaller than the RMSE of the prices from last year (0.233). In the context of this analysis, \$0.167 is the square root of the mean of the squared differences between the actual per-unit retail prices of avocados and our predicted prices. Given that the actual average per-unit price of avocados can range from around \$1.0 to \$1.6, we consider an RMSE of \$0.167 an acceptable value given our limited data.

We have included the table of estimated parameter values in Appendix ?, Table ?.

# Discussion
In this project, we used regression models with SARIMA errors to predict the sales of Hass avocados. As discussed in the previous two sections, we believe we have reasonable addressed our two research goals. Specifically, we are able to predict the total sales volume of conventional avocados in the U.S. from April 2017 to March 2018 with a RMSE of 2.805 million avocados. We then predicted the average per-unit retail price of conventional avocados in the U.S. from April 2017 to March 2018 with a RMSE of $0.167.

One of the main strengths of this analysis is that we have accounted for the autocorrelation structure and the seasonal pattern in the errors of our regression models. In addition, our predictions are somewhat accurate given our very limited data. Our data are limited in two ways. First, we only have a little more than three years (39 months) of data. Given that we have decided to make prediction on an entire year, we only have 27 months to fit the model. Second, even though there are 13 variables in the avocado dataset, many of them are linear combinations of each other and are thus not usable in the model. For instance, the total sales volume is the sum of the sales volume of small, large, and extra large avocados. The additional macroeconmic measures unemployment rates and CPI turned out to be highly correlated.In future analysis, we hope to incorporate additional predictors, such as supply volume, import/export volume, tariff, weather to better inform our prediction.

Beyond the limitation in our data, there is another important limitation with a regression model approach: if we want to make predictions about the future (instead of the known 04/2017-03/2018 time period), we would need data for the predictors in the future. The actual values are certainly not accessible, but we can potentially obtain plausible predictions. For instance, organizations such as the Federal Reserve releases their forecasts of marcoeconomic measures including the unemployment rate [@Unemployment_rate_projections]. We can use these prejections to predict the per-unit prices and subsequently use our predicted prices to forecast the sales volume.

An interesting extension of this study would be to use the newly available 2018-2020 Kaggle avocados dataset to investigate the avocado retail sales before and during the pandemic. 2019 is a special year because earlier that year, tariffs and transportation-related delays contributed to avocado supply chain disruptions and consequently, unusually high prices [@Transportation_disruption;@Tariffs_disruption]. On the other hand, the avocado industry is remarkably resilient during the pandemic. It is said that the demand for avocados surged during the pandemic since people are paying more attention to their health and wellness, and more people are cooking at home [@Pandemic_proof]. In either case, the prediction problem would be more challenging since if we simply use the current approach, we would likely under-predict the price and/or the sales volume. 

# Appendix

## Using the \texttt{forecast} package
When using the \texttt{Arima()} function, we can specify the non-seasonal component using the \texttt{order} argument, the seasonal component using the \texttt{seasonal} argument, and we can specify our predictors as a $n\times p$ matrix in the \texttt{xreg} argument where $n$ is the number of the observations and $p$ is the number of predictors.
...
The VIF values for all three predictors are all within the commonly acceptable range (i.e. less than 5) [@VIF_range]. However, according to the the correlation plot below, the unemployment rate seems to be highly negatively correlated with the CPI.

```{r fig.height = 4, fig.width = 6, fig.cap="Unemployment Rate and CPI Highly Correlated"}
...
The VIF values per-unit price and unemployment rate are still small in this updated model. Furthermore, the magnitude of the correlation between the two predictors is 0.523 < 0.7, suggesting no severe multicollinearity [@Common_cutoff].
...
An additive decomposition model for a time series $x_t$ can be represented as $$x_t = m_t + s_t + e_t$$ where at time $t$, $m_t$ is the trend, $s_t$ is the seasonal effect, and $e_t$ if a random error assumed to have a mean of zero and correlated over time [@Decomposition].
...
```{r fig.height = 3, fig.width = 6.5, fig.cap="Seasonal Panel of Decomposition Plot"}
...
We observe a seasonal pattern repeated annually according to Figure?.

```{r fig.height = 3.5, fig.width = 6.5, fig.cap="Times Series, ACF, and PACF Plots of Residuals"}
ggtsdisplay(q1.lm1.residuals, points = FALSE, lag.max = 39,
            main = "Times Series, ACF, and PACF Plots of Residuals",
            ylab = "Residuals")
```
According to the ACF and PACF plots, there seems to be evidence of autocorrelation in the residuals.

### Parameter Selection for the SARIMA Process
Referring back to the ACF plot in Figure ?, we observe a pattern similar to a sinusoidal oscillation in the ACF plot, which is suggestive of a seasonal component and an autoregressive structure [@Sinusoidal_oscillation]. We also notice the spikes at lag 12 and 24. Thus, we would like to check if we should use seasonal differencing.
...
kable(q1.D.table, digits = 3, caption = "Comparing Models", col.names = c('p', 'd', 'q', 'P', 'D', 'Q', 'AIC', 'AICc', 'BIC'))
```

As shown in Table ?, adding the seasoning differencing greatly reduces the AIC, AICc, and BIC values. AIC (Akaike Information Criterion), AICc (corrected Akaike Information Criterion), and BIC (Schwarz's Bayesian Information Criterion) are both commonly used model selection criteria. Compared to AIC, BIC tends to favor more parsimonious models [@BIC_vs_AIC], while AICc is said to be preferred when the sample size is small [@AICc_sample_size]. In this case, all three criteria agree. Thus, we proceed with model with a seasonal difference (i.e. $D = 1$).

```{r fig.height = 3.5, fig.width = 6.5, fig.cap="Times Series, ACF, and PACF Plots of Residuals"}
ggtsdisplay(q1.sarima.000.010$residuals, points = FALSE, lag.max = 27,
            main = "Times Series, ACF, and PACF Plots of ARIMA(0,0,0)(0,1,0)[12] Residuals")
```
We check the residuals of the regression with ARIMA(0,0,0)(0,1,0)[12] errors. Ideally, these residuals should resemble white noise. For a white noise series, we usually expect 95% of the spikes in the ACF to lie within $\pm\frac{2}{\sqrt{T}}$, where $T$ is the length of the time series [@White_noise_ACF]. The blue dashed lines represent these bounds, in this case $\pm\frac{2}{\sqrt{27}} \approx \pm 0.385$. However, observing the ACF and PACF plots in Figure ?, there still seem to be AR or MA structures in the residuals.
...
kable(q1.PQ.table, digits = 3, caption = "Comparing Models", col.names = c('p', 'd', 'q', 'P', 'D', 'Q', 'AIC', 'AICc', 'BIC'))
```

In this case, according to Table?, the criteria values for the three models are relatively similar, with the $(0, 0, 0) \times (1, 1, 0)_{12}$ model having slightly smaller values compared to the other two models.

```{r fig.height = 3.5, fig.width = 6.5, fig.cap="Times Series, ACF, and PACF Plots of Residuals"}
ggtsdisplay(q1.sarima.000.110$residuals, points = FALSE, lag.max = 27,
            main = "Times Series, ACF, and PACF Plots of ARIMA(0,0,0)(1,1,0)[12] Residuals")
```

The residual plots of the three models are very similar. Here, we included the residual plots of the regression with ARIMA(0,0,0)(1,1,0)[12] errors in Figure ? for reference. We notice that even after accounting for seasonal AR or MA components, there still seem to be autocorrelation in the residuals. Specifically, we observe a spike at lag 1 that exceeds the dashed blue line in the PACF plots with $P = 0$ [Figure ?] or $P = 1$ [Figure ?]. This suggests a possible non-seasonal AR(1) component (i.e. $p = 1$) [@PACF_lag_1].
...
$$\text{Per-Unit Price}_i = \beta_0 + \beta_1 \ \text{Total Volume}_i + \beta_2 \ \text{Unemployment Rate}_i  + \epsilon_i, \: \epsilon_i \sim \mathcal{N}(0, \sigma^2)$$ 
...
```{r fig.height = 3.5, fig.width = 7.5, fig.cap="Predicting Avocado Per-Unit Price from 2017-2018"}

...
```{r fig.height = 3.1, fig.width = 6, fig.cap="Time Series Plot of Per-unit Prices of Conventional Avocados in the U.S."}
# time series plots: 
...
The per-unit price of the conventional Hass avocados increases every year from 2015 to 2018 and exhibit easonal trends: prices peak during in third quarter and drop to the lowest in the first quarter. Organic avocados have very similar results.
...
ggplot(data = df.total.conventional, aes(x = Total.Volume, y = Average.Price)) +
  geom_point() +
  labs(title = 'Potential Inverse Relationship between Per-Unit Price and Total Volume',
       x = 'Total Number of Avocados Sold',
       y = 'Per-Unit Price ($)')
```

A inverse relationship seems to exist between the per-unit price and the total sales volume of the conventional Hass avocados.
...
{plot(fmanweek1, main = "Prediction with Uncertainty", xlab = 'Weeks (2015-2018)')
lines(ts(avg_price_in_weeks))}
```

The predicted per-unit prices for the first 10 weeks and between week 35 and week 45 are relatively close to the observed prices (differ by less than $0.1). The predicted prices between week 25 and 35 exhibit a similar pattern to the actual prices between week 20 and 30 - the predictions seem to lag by roughly 5 weeks.

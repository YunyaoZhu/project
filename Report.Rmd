...
{plot(fmanweek1, main = "3 Year Prediction with Uncertainty", xlab = 'Weeks (01/2015-03/2018)')
...
```{r fig.height = 3.5, fig.width = 7.2, fig.cap="Predicting the Monthly Per-Unit Conventional Avocado Prices from 2017-2018"}
man.fit.month <- Arima(avg_price_in_months[1:27], xreg=(df.total.conventional.month[["Total.Volume"]])[1:27], order = c(3,0,0), seasonal = list(order = c(0,1,1), period = 12))
man.pre.month <- predict(man.fit.month, newxreg=(df.total.conventional.month[["Total.Volume"]])[28:39], n.ahead=12)
...
par(mai=c(0.8,0.5,0.8,0.2))
fmanmonth1 <- forecast(man.fit.month, xreg=(df.total.conventional.month[["Total.Volume"]])[28:39], h=12)
{plot(fmanmonth1, main = "3 Year Prediction with Uncertainty", xlab = 'Months (01/2015-03/2018)')
